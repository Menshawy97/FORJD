...
export const exercises = pgTable(
  ...
  (table) => [
    ...
    /**
     * The list endpoint's sort key (Phase E). `listExercises` orders by `(name, id)` and
     * resumes with a row-constructor comparison against that same pair -- which is the whole
     * point of keyset pagination, but a keyset without a matching index is just a full sort
     * of the match set on every page, which is what offset pagination does and what the
     * keyset was chosen to avoid.
     *
     * Both columns in this order: `name` alone leaves the tiebreaker unsorted, and the range
     * comparison is on the pair.
     *
     * A plain btree, unlike the tsvector and trigram indexes in migration 0006, *is*
     * expressible in this typed schema -- so it belongs here and is generated by drizzle-kit
     * rather than hand-written (CLAUDE.md rule 14).
     */
    index("exercises_name_id_idx").on(table.name, table.id),
  ],
);
