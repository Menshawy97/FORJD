...
export const profiles = pgTable(
  'profiles',
  {
    ...
    /**
     * A real column (ADR-019), separate from `displayName` -- the design shows both
     * simultaneously ("James Mitchell" above "@jmitch"), so they are two fields, not one
     * rendered twice. Nullable because every account that predates this field has no honest
     * value to backfill; the `pickUsername` onboarding screen fills it for new accounts, and
     * existing accounts are prompted from `edit-profile`, not blocked. Case-insensitive
     * uniqueness is enforced below via `profiles_username_unique`, not by this column -- a
     * column-level constraint cannot see other rows. Format (`/^[a-z0-9_]{3,20}$/`) is
     * validated at the API boundary (`usernameSchema` in @forjd/contracts), not here.
     */
  ...
  (table) => [
    /**
     * Case-insensitive global uniqueness (ADR-019), mirroring the `exercises_owner_name_unique`
     * pattern in exercises.schema.ts. Global, not owner-scoped -- unlike a custom exercise
     * name, a username has no owner to scope by; it identifies the account itself. A partial
     * index (`where username is not null`) rather than a plain unique constraint, so the many
     * pre-ADR-019 accounts with a null username never collide with each other -- Postgres
     * already treats NULL as distinct from NULL in a unique index, and the `where` clause keeps
     * that behaviour explicit and consistent with the rest of this codebase's partial indexes
