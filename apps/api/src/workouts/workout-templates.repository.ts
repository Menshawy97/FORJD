 ...
 *
 * Shared with `workout-sessions.repository.ts`, which imports it from here rather than
 * duplicating it -- both halves narrow the same handful of `NOT NULL` vocabulary columns
 * (`activity`, `status`, `type`, `measure`).
 */
export function keepKnownOrFallback<T extends string>(
  value: string,
  known: readonly T[],
  fallback: T,
): T {
...
export function keepKnownNullable<T extends string>(value: string | null, known: readonly T[]): T | null {
 ...
 * `WorkoutSessionsRepository` behind an unchanged `WorkoutsRepository` facade -- see that
 * file's own docblock.
 */
@Injectable()
