...
import { apiErrorCode, classifyRequestFailure, OFFLINE_MESSAGE } from '@/auth/failure';
...
export default function ConnectScreen() {
  ...
  const handleConnectWhoop = async () => {
    ...
    try {
      ...
      // ADR-043: the server will not start WHOOP until health-data consent is given. That is
      // not an error to report -- it is the moment to ask, so go to the explainer.
      if (apiErrorCode(cause) === 'health_data_consent_required') {
        // Cast: the typed-routes union is generated by the dev server and is stale until the
        // next `expo start`, so a brand-new route is not in it yet.
        router.push('/health-consent' as Href);
      } else {
        toast.show(describeFailure(cause));
      }
    } finally {
