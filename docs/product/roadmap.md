...
**PR [#144](https://github.com/Menshawy97/FORJD/pull/144) — a real production bug, found and fixed the same session, per CLAUDE.md's own post-merge discipline.** Watching the post-merge deploy after 7F (as the merge-discipline section requires) showed the real staging Cloud Run deploy had started failing: "container failed to start and listen on the port... within the allocated timeout." Root cause: `WhoopModule` (7F) put three providers — `token-cipher.provider.ts` (7A), `whoop-client.ts`'s and `whoop-oauth.service.ts`'s factories — permanently into `AppModule`'s DI graph, and all three called `ConfigService.getOrThrow` at eager module construction. No real environment has WHOOP credentials or a token-encryption key configured yet (decision 1: this phase is built against recorded payloads, not live WHOOP access), and `deploy-api.yml` never provisions these as secrets — so the eager `getOrThrow` calls crashed the **entire API's boot**, not just WHOOP's own routes, in every environment. Fixed by switching all three to `config.get(key, fallback)`, deferring "is this configured" to first actual use. **Verified against the real deployed service, not just CI**: `curl https://forjd-api-staging-772363715082.us-central1.run.app/api/v1/health` → `200 {"status":"ok","database":"up"}` after the hotfix's own deploy completed. Staging had been stuck on the pre-7F revision for roughly 8 hours before this landed.

**The lesson, worth carrying into any future phase that registers a new module into `AppModule`:** an eager `useFactory` that calls `getOrThrow` is a statement that this config is *always* required for the API to run at all — correct for Supabase/OpenAI (every environment has those), wrong for an integration a phase deliberately builds without live credentials yet. Check which is true before adding a new eager provider, and prefer `config.get(key, fallback)` with the validation moved to first real use whenever the answer is "not yet."

**Still outstanding, correctly so:** the live WHOOP OAuth round-trip has still never run (decision 1, unaffected by either PR this session). The Phase 6F Health Connect physical-device test (ADR-035) remains untouched.

**Next: slice 7H** — conformance rules, ADR-037 (recording the whole Phase 7 integration shape, including this session's lazy-config lesson), and the doc-hygiene sweep (`integrations.md`'s stale `Flutter → Your API → WHOOP OAuth` line, `system.md`'s stale `mobile/ Flutter` line). That closes Phase 7.

### Session close, 2026-09-09 (Phase 7 slice 7F: routes, webhook, raw-body capture — Phase 7's code is live over HTTP for the first time)

**PR [#141](https://github.com/Menshawy97/FORJD/pull/141) merged and confirmed green on `main`.** `WhoopModule` is now registered in `AppModule` — `POST /api/v1/integrations/whoop/{authorize,sync}`, `DELETE /api/v1/integrations/whoop`, `GET /api/v1/integrations/whoop/callback`, and `POST /api/v1/integrations/whoop/webhook` all exist and are reachable, following `auth.controller.ts`'s public-class-with-per-route-guard shape.

**What shipped:** `whoop-callback.service.ts` (completes the OAuth round trip via `oauth_state`, fetching WHOOP's own profile specifically to learn its numeric user id — the only thing a later webhook carries that identifies the internal user), `whoop-token-access.ts` (the token-refresh policy extracted out of `WhoopProvider` so the webhook path shares it instead of duplicating it), `whoop-webhook-signature.ts` (WHOOP's documented HMAC scheme, timing-safe, 5-minute freshness window), `whoop-webhook.service.ts` (one event end to end, synchronous per decision B), `whoop-sync.service.ts` (the "sync orchestrator" `health-provider.ts`'s own docblock named as not yet built — asks the provider its own capabilities rather than hardcoding a metric list). `main.ts` gains `rawBody: true`. `health-data.module.ts` now exports `HealthDataRepository` so this new module reuses the existing ingestion singleton.

**One real bug found and fixed by the e2e test hitting real Postgres, not by any unit test:** a WHOOP recovery yields two metrics (`hrv`, `resting_heart_rate`) sharing one `cycle_id`, and a WHOOP sleep yields six sharing one `id` — but `health_observations`' idempotent-reingest index is `(user_id, source, provider_record_id)`, with no `metric_type` column at all. Upserting several rows with the same unqualified id in one batch made Postgres reject the whole statement ("ON CONFLICT DO UPDATE command cannot affect row a second time"). This is structurally different from Health Connect, where every record type already maps to exactly one metric, so the collision never had a chance to surface there. Fixed in `whoop-record-mapping.ts` (from 7D, already on `main`) by composing `providerRecordId` with the metric type, e.g. `"93845:hrv"` — every mapping unit test that asserted the old bare-id format was updated to match. **Worth remembering for any future multi-metric-per-record provider:** a mocked-repository unit test cannot catch a real database constraint collision; only a spec that performs the actual upsert (the schema spec, or here, the e2e spec) can.

**`whoop.e2e-spec.ts` proves:** unauthenticated rejection on every authenticated route, the full authorize → callback round trip actually completing and connecting the account, a callback with no matching pending state redirecting to an error status, cross-user isolation (the other user's `sync`/`disconnect` never touch the owner's connection), a forged webhook signature rejected, a stale-timestamp webhook rejected even when otherwise correctly signed, and a validly signed webhook being ingested and safely idempotent on re-delivery.

**CI note:** `WhoopModule`'s providers (`token-cipher.provider.ts`, `whoop-client.ts`, `whoop-oauth.service.ts`) all call `config.getOrThrow` at eager module construction, and the module is now always part of `AppModule`'s DI graph — `.github/workflows/ci.yml` needed five new placeholder env vars (`TOKEN_ENCRYPTION_KEY`, `WHOOP_CLIENT_ID`, `WHOOP_CLIENT_SECRET`, `WHOOP_REDIRECT_URI`, `WHOOP_WEBHOOK_SECRET`) added to the `API lint, build, test` job's `env:` block, matching the existing Supabase/OpenAI placeholder pattern exactly (well-formed but fake values; no test in CI ever performs a real WHOOP or Supabase call).

**Still outstanding, correctly so:** the live WHOOP OAuth round-trip has still never run against WHOOP's real servers (decision 1 from the plan — everything here is built and tested against recorded/synthetic payloads and a faked `WhoopClient`). The Phase 6F Health Connect physical-device test (ADR-035) remains untouched and unrelated. `*.deleted` webhook events are acknowledged but do nothing — `health_observations` has no delete path for any provider yet, a gap recorded rather than hidden (see the closing ADR-037, still to be written in 7H).

**Next: slice 7G** — the mobile Connect screen (`FORJD Mobile.dc.html`'s `s_connect()`), built pixel-by-pixel. Only the WHOOP card gets wired this phase; Apple Health and Health Connect render inert per the plan's decision. Then **7H**: conformance rules, ADR-037, and the doc-hygiene sweep (`integrations.md`'s stale `Flutter → Your API → WHOOP OAuth` line, `system.md`'s stale `mobile/ Flutter` line).
...
**What shipped:**
- **7A** — `apps/api/src/common/crypto/token-cipher.ts`: the first encryption code anywhere in this repo, AES-256-GCM with key-versioned ciphertext, 100% coverage. `token-cipher.provider.ts` (the injectable `TOKEN_CIPHER`) is written but not yet wired into any module — nothing calls it until 7D. ADR-036 records AES-256-GCM-in-app-code over Cloud KMS.
- **7B** — `packages/domain/src/external-connection-vocabulary.ts` (`EXTERNAL_CONNECTION_PROVIDERS`/`STATUSES`) and `apps/api/src/database/schema/external-connections.schema.ts`, migration `0016`. A genuinely separate table from `health_connections`, per the plan's decision 3. Unique index on `(user_id, provider)` deliberately does double duty as the lazy-refresh lookup index too — no redundant second index.
- **7C** — `HealthProvider`/`SyncedObservation`/etc. and the shared contract-test suite moved from `apps/mobile/src/integrations/health/` to `packages/domain/src/health-provider.ts` / `health-provider-contract.ts`. Git recognized both as renames (89%/64% similarity). Every mobile file that used the local copy now imports from `@forjd/domain` instead — no behaviour change, confirmed by identical test counts before/after (47/47 in the health-integrations directory, 175/175 domain, 890/890 mobile-wide individual tests).

**Two real environment issues hit and worked through, neither a code defect:**
- **`drizzle-kit migrate`'s CLI spinner hangs indefinitely in this session's sandboxed Windows shell** (confirmed: the underlying Postgres connection itself is fine; the spinner render loop simply never resolves in this specific non-interactive wrapper). Worked around locally by applying the generated SQL directly via `docker exec -i ... psql` and inserting the matching sha256-hash row into `drizzle.__drizzle_migrations` by hand — a **local-only workaround**; the committed migration file itself is standard and PR #135's CI run confirmed `pnpm --filter @forjd/api db:migrate` applies it cleanly and normally on GitHub's Linux runner. If a future session hits the same hang, don't fight the CLI — apply the SQL directly and verify against CI, the same way.
- **This machine's Node resolves `localhost` to a non-routable IPv6 loopback for the local Postgres container**, hanging any Postgres-backed Jest spec that uses the default `DATABASE_URL` (`localhost:5432`) — a *new* symptom this session, not present in the Phase 6 close notes' documented flakiness (which was pure contention/timeout under parallel load, not an infinite hang). Root-caused precisely: `postgresql://...@127.0.0.1:5432/...` connects in ~70ms; `postgresql://...@localhost:5432/...` never resolves. Worked around by exporting `DATABASE_URL` with `127.0.0.1` for local verification only — **no repo file was changed**, since every spec's committed default correctly stays `localhost` (CI's Linux runners don't have this quirk). If a future session on this machine sees a Postgres spec hang instead of merely time out, check this before assuming it's the known contention flake.

**Still outstanding, correctly so:**
- `TOKEN_CIPHER` (7A) has no consumer yet — becomes a real boot-time requirement (`TOKEN_ENCRYPTION_KEY` required) only once 7D's WHOOP OAuth service starts injecting it.
- Everything the plan already lists as owed when Phase 7 closes (the live WHOOP OAuth round-trip, since decision 1 was to build against recorded payload shapes; the Phase 6F Health Connect physical-device test, ADR-035, untouched by this phase) is unaffected by 7A-7C and remains open.

**Next: slice 7D** — the WHOOP client, OAuth service, and record-mapping functions in `apps/api/src/integrations/whoop/` (first occupant of that directory), tested against recorded real WHOOP v2 payload fixtures. See `phase-7-plan.md`'s 7D entry for the exact file list.

### Session close, 2026-09-08 (Phase 6, post-6F: readiness scoring + light-up-the-UI)

**Two PRs merged and confirmed green on `main`: [#131](https://github.com/Menshawy97/FORJD/pull/131) (readiness domain/contracts) and [#132](https://github.com/Menshawy97/FORJD/pull/132) (readiness API endpoint + mobile UI wiring, plus a follow-up UI-polish commit from live-device review).** This closes the "Post-6F — light up the UI" section of `phase-6-plan.md` — every seam that section named (`readiness-card.tsx`'s prop, `stat-strip.tsx`'s four health props, `progress.tsx`'s Health tab, `step-count-card.tsx`'s prop) is now real. ADR-031 (readiness methodology) is **Accepted**, no longer Proposed.

**What shipped:**
- `packages/domain/src/readiness.ts` — `computeReadiness()`: HRV, resting heart rate, sleep duration, and respiratory rate scored against each metric's own non-overlapping 60-day baseline vs. 7-day recent window, with Hopkins (2000)'s Smallest Worthwhile Change (0.5×SD) and WHOOP's published red/yellow/green zone cutoffs. Withheld with a stated reason under ~30 days of history. 32 boundary tests found and fixed 3 real bugs (near-zero-variance saturation, off-by-one window boundaries, overlapping-window signal dampening) — see the file's own docblock for full citations and the "FORJD's own uncited design choices" it explicitly separates out.
- `packages/domain/src/health-insight.ts` — `evaluateHealthInsight()`: a real, cited "FORJD Insight" (never "AI Insight" — ADR-030) that names the most-deviated readiness component, citing Plews et al. 2013 only for a "low" reading, never for "elevated" (no citation needed for good news).
- `walking_heart_rate` added to the domain vocabulary with **no** Health Connect mapping (`Partial<Record<HealthMetricType, RecordType>>` on `METRIC_TO_RECORD_TYPE`) — `HEALTH_CONNECT_SUPPORTED_METRICS` is derived by filtering the mapping's own keys, so it's hidden on Android by a capability flag, never a `Platform.OS` check. Reappears automatically the day an Apple Health provider (Phase 11) can supply it.
- `GET /health-data/readiness` (`apps/api/src/health-data/`): computes the composite from resolved observations, bucketed into the caller's own local calendar days via a `timeZone` query param.
- Mobile: Home's readiness card gets a real score/zone/component chips with a ring-fill animation that replays on every focus, not just first mount (`useFocusEffect`, mirroring the prototype's `@keyframes fj-grow .8s ease-out`). Stat strip gets real sleep/HRV/RHR/steps. Progress's new Health tab (`features/health/health-view.tsx`) gets real metric tiles, a swappable-widget picker, a trend chart, and a FORJD Insight card. Step-count card gets a real daily total.

**The ring animation was built from the prototype's static CSS source, not from watching the user's own reference video** — the `Read` tool cannot open `.mp4` files, and the live prototype preview only renders a static, non-interactive snapshot outside the project directory. The user has since tested it live on their iPhone via plain Expo Go (see below) without objection, so treat it as tacitly confirmed, but if a future session touches this file, know that no one ever explicitly said "yes, this matches" in words.

**Live-device UI review found several deliberate deviations from the design screenshots, all by explicit user direction** (commit on top of #132, `fix(6h)`):
- No accent border or highlighted background on the selected headline tile in `health-view.tsx` (the design shows both).
- "Health metrics" heads the trend chart too, not just the list below it — the design's own untitled section groups the trend chart with the headline tiles instead.
- `hrv` and `resting_heart_rate` stay selectable as headline tiles via "Choose widget" but now also always appear in the metrics list below (previously each metric was either a tile or a list item, never both).
- The trend chart's own title now matches the metric-card title style (font, size, weight, sentence case) instead of the small uppercase caption style the design uses.
- Health and Body tabs' first content row no longer carries an extra `marginTop: 12` that the Strength tab's own first row doesn't have — this closed a spacing mismatch the user caught by comparing screenshots side by side live on device.

**Two real bugs found and fixed mid-session, neither caused by the readiness work itself:**
- `apps/mobile/src/features/health/health-metrics.spec.ts`'s `sumForLocalDate` tests used UTC timestamps near a day boundary (e.g. `23:00Z`), which silently rolled onto the wrong calendar day on this machine's `Africa/Cairo` timezone. Mutating `process.env.TZ` at runtime does **not** fix this — jest-expo's test environment resolves the host timezone once, before test code runs, and does not re-read it. Fixed by moving the fixture timestamps to unambiguous mid-day UTC values instead. If a future spec needs calendar-day logic tested against real `Date` objects, use mid-day UTC fixtures, not boundary-adjacent ones.
- Six existing test suites (`home-fidelity`, `home-nutrition-card`, `home-stats`, `login`, `progress-fidelity`, `swipe-back-login`) had `@/auth/apiClient` jest mocks that predated `getHealthObservationSeries`/`getReadiness`, which `HomeScreen`/`ProgressScreen` now call — an absent mock throws a `TypeError` before `Promise.allSettled` can even settle it, unlike a rejected one. All six updated with rejected/resolved mocks matching the existing `getWorkoutStats` pattern in the same files.

**A stale local build was the source of a false-alarm typecheck failure** early in this session: `apps/api`'s `tsc --noEmit` failed on `walking_heart_rate` not being assignable, purely because `packages/domain`'s and `packages/contracts`' compiled `dist/` output predated the domain change. Rebuilding both (`pnpm --filter @forjd/domain build && pnpm --filter @forjd/contracts build`) fixed it with no code change needed — worth checking before chasing a "real" type error after editing a workspace package.

**PR-per-slice discipline, adapted for this size of work:** split into two sequential PRs rather than one — #131 (domain + contracts, self-contained and independently testable) merged and confirmed green on `main` *before* #132 (API + mobile, which depends on #131's types) branched. This mirrors 6A–6D's own sequencing rather than the usual single-slice-per-PR pattern, because the two pieces have a real compile-time dependency in this monorepo (a mobile/api branch cut before #131 merges would fail the exact same stale-type check the local build hit above).

**Local-machine test flakiness encountered and worked through, not fixed in code:** this session ran an unusually heavy load of parallel Jest processes (full mobile suite, full API suite with real Postgres, EAS CLI calls, a live Metro server) on one machine, which produced two classes of flake, both confirmed non-issues before proceeding:
- Mobile: `act()`-warning/30s-timeout failures that varied which suite tripped from run to run — this repo's own `apps/mobile/jest.config.js` comment already documents this pattern for `renderRouter()`-heavy suites under worker contention.
- API: `apps/api/src/database/schema/health-data.schema.spec.ts` and similar Postgres-integration specs hit their 5000ms default timeout under load — confirmed via `git stash` that this happens identically on a clean checkout with zero readiness-slice changes, and CI (which runs alone, not under this session's contention) passed the identical commit minutes earlier.

**Still outstanding, correctly so — not a blocker for anything just merged:**
- **The Phase 6F physical-device test (ADR-035) is still owed.** Checked explicitly this session: no mobile UI screen calls `HealthConnectProvider.connect()`/`.sync()` yet (only `stat-strip.tsx` and `step-count-card.tsx`'s docblocks reference it, as a "nothing populates this yet" note) — this session's UI work only wired the *read* side (fetching already-ingested observations/readiness from the API), so ADR-035's actual trigger condition ("before any UI screen wires real users to this provider") has still not been crossed. The device test becomes urgent the day a "Connect Health Connect" button or an automatic sync trigger is built, not before.
- The readiness-card ring animation, while tested live via Expo Go, was never explicitly confirmed word-for-word against the two videos the user originally sent (see above).

**Environment note for whoever resumes:** the user has no paid Apple Developer Program account, so there is no signed iOS dev-client build and none is planned for now. Testing on the user's iPhone happens via the free **Expo Go** app against a plain `npx expo start --host lan` server (not `--dev-client`) — this works because `react-native-view-shot` (the one dependency that forced ADR-028's dev-client requirement) is only imported by the Nutrition/Workout **share-card** screens specifically, which Expo Go cannot open (they'll crash) but every other screen, including everything touched this session, works fine. If a future session needs the share-card screens tested on iOS, that conversation needs to happen with the user first — it is a real cost/credential decision (Apple Developer Program, $99/yr), not something to route around silently.
...
3. **Phase K — programs. K1–K5 done (PRs #99, #100, #103, #109, #110, #111); K6 next** —
   ...
   **K3 is complete.** `POST /programs/:id/enrol` and `DELETE /programs/enrollment`. Enrolling
   while already following something else ends the previous enrolment and starts the new one in
