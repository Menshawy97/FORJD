...
| 7 — WHOOP | 29-30 | OAuth, webhooks, adapter | **In progress** — [planned](phase-7-plan.md); 7A-7F merged. Routes are live: `POST /integrations/whoop/{authorize,sync}`, `DELETE /integrations/whoop`, `GET /integrations/whoop/callback`, `POST /integrations/whoop/webhook`. Remaining: 7G (mobile Connect screen), 7H (conformance, ADR-037, doc hygiene) |
